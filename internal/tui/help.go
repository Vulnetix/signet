...
func keySections() []keySection {
	return []keySection{
		...
		{"agents screen (/agents)", []keyBinding{
			{"1, 2, 3, tab, shift+tab", "switch between running, profiles and audit"},
			{"up, down", "move the highlight"},
			{"enter", "running and audit: follow the agent's thread in chat; profiles: edit"},
			{"s", "profiles: start the highlighted profile in the background"},
			{"p", "running: pause or resume a background agent"},
			{"x", "running: stop a background agent, cancel or dismiss a subagent"},
			{"a", "running: show the highlighted agent's audit trail"},
			{"c", "audit: show every agent again"},
			{"esc", "back"},
		}},
		{"runs panel (after f8/f9)", []keyBinding{
			...
			{"x", "activity: kill; subagents: cancel or dismiss; processes: stop"},
			{"r", "processes: restart the selected process"},
		...
		{"/model", []keyBinding{
			...
			{"p", "open /providers"},
			{"tab", "cycle model mode between routed and defined"},
			{"esc", "back"},
		}},
