...
func (g *Grep) Execute(ctx context.Context, args map[string]any) (Result, error) {
	...
	if g.rgPath != "" {
		backend = "rg"
		out, _ = g.runRg(ctx, pattern, sub)
	} else {
		out, _ = g.runGrep(ctx, pattern, sub)
	}
	...
	lines := normaliseGrepLines(out, maxLineLen, maxMatches)
	return GrepResult(strings.Join(lines, "\n"), backend), nil
}

func (g *Grep) runRg(ctx context.Context, pattern, sub string) ([]byte, error) {
...
func (g *Grep) runGrep(ctx context.Context, pattern, sub string) ([]byte, error) {
...
func normaliseGrepLines(out []byte, maxLineLen, maxMatches int) []string {
	if len(out) == 0 {
		return nil
	}
