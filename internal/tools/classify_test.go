...
// command, a page written off this machine, a search result, and a file whose
// bytes the harness did not write.
func TestArbitraryContentKindsClassify(t *testing.T) {
	for _, k := range []Kind{KindBash, KindWebFetch, KindWebSearch, KindRead} {
...
// Edit return a confirmation the harness wrote itself, and a native runs a
// fixed argv the harness built.
func TestShapedKindsDoNotClassify(t *testing.T) {
	for _, k := range []Kind{KindGrep, KindGlob, KindWrite, KindEdit, KindNative} {
		if k.NeedsClassifier() {
...
// The set is exactly the four arbitrary-content kinds. Pinning it whole means
// a new kind cannot be added to either side without this test being updated
// on purpose.
func TestClassifierKindsIsExactlyTheArbitraryContentSet(t *testing.T) {
	want := map[Kind]bool{KindBash: true, KindWebFetch: true, KindWebSearch: true, KindRead: true}
