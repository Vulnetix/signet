...
the questionnaire appears within seconds with options naming real top-level
files, `space` selects options, `n` adds a note, `s` skips a question, `enter`
on an option picks it and submits, the answers appear in the transcript, the
same question is not asked again, and the plan uses the file you picked. Press `esc` at the questionnaire
